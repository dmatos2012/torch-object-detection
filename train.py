...
def create_datasets_and_loaders(transform_train_fn=None):        
        ...
        loader_train = create_loader(dataset_train,  input_size, batch_size,
                        interpolation=interpolation,
                        fill_color=fill_color,
                        num_workers=num_workers,
                        is_training=True)

        loader_val = create_loader(dataset_val,  input_size, batch_size,
                        interpolation=interpolation,
                        fill_color=fill_color,
                        num_workers=num_workers,
                        is_training=False)
...
for input, target in loader_train:
        ...
        torchvision.utils.save_image(list(input),
                        "train-batch.jpg",
                        padding=0,
                        normalize=True)
